...
export default function StepTurnOrder({
  ...
  // Only perform initial draw if turnOrder is empty or does not match selected meeples count
  useEffect(() => {
      ...
      <div className="bg-slate-900/90 rounded-3xl p-5 sm:p-6 border border-slate-800 shadow-xl space-y-5">
        {isShuffling ? (
          <div className="py-10 space-y-3">
            <Shuffle size={40} className="mx-auto text-amber-400 animate-spin" />
            <p className="text-xs font-bold text-slate-300">Tirage royal en cours...</p>
          </div>
        ) : (
          <>
            {/* Standard Players Order */}
            ...
            <div className="pt-2">
              <button
                type="button"
                onClick={performDraw}
                className="text-xs font-semibold text-slate-400 hover:text-amber-300 flex items-center justify-center gap-1.5 mx-auto py-1 px-3 rounded-xl hover:bg-slate-800/60 transition-all"
              >
                <RotateCcw size={13} />
                Re-tirer un ordre aléatoire
              </button>
            </div>
          </>
        )}
