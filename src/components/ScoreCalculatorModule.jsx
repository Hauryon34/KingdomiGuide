...
export default function ScoreCalculatorModule({
  playerCount,
  selectedMeeples,
  playerNames,
  gameMode,
  ...
  const defaultSize = gameMode === 'duel' && playerCount === 2 ? 7 : 5;
  const [gridSize, setGridSize] = useState(defaultSize);
  
  // Grids storage for each active player
  const [playerGrids, setPlayerGrids] = useState(() => {
    const grids = {};
    selectedMeeples.slice(0, playerCount).forEach((id) => {
      grids[id] = createEmptyGrid(defaultSize);
    });
    return grids;
  });

  // Active player selected in tabs
  const [activePlayerId, setActivePlayerId] = useState(selectedMeeples[0] || 'blue');

  // Paintbrush selection state
  ...
  // Active players list
  const activePlayers = selectedMeeples.slice(0, playerCount).map((meepleId, index) => {
  ...
  // Handle cell click (brush apply or crown cycle)
  const handleCellClick = (r, c) => {
    ...
    if (selectedTerrain === 'empty') {
      ...
      // If clicking same terrain, cycle crowns
  ...
  // Prepare ranked players for podium
  const rankedPlayers = [...activePlayers].sort(comparePlayers);

  return (
    <div className="space-y-4 pb-24 max-w-lg mx-auto px-1 animate-fade-in">
      {/* 1. Sélecteur de Joueur (Compact & Visuel) */}
      ...
      {/* 2. Bandeau Score Total du Joueur Actif */}
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 rounded-2xl p-3.5 border border-slate-800 shadow-lg flex items-center justify-between">
        ...
        <div className="text-right">
          <div className="text-2xl sm:text-3xl font-black font-medieval text-amber-300 leading-none">
      ...
      <div className="bg-slate-900/95 rounded-2xl p-3 border border-slate-800 shadow-md space-y-2.5">
        ...
        {/* Sélecteur de couronnes par défaut */}
      ...
      {/* 5. Grille Interactive Mobile-Optimized */}
      <div className="flex justify-center p-2.5 sm:p-4 bg-slate-900/90 rounded-3xl border border-slate-800 shadow-xl overflow-x-auto">
      ...
      {/* 6. Alertes de Règles & Connexions */}
      ...
      {/* 7. Bonus Officiels (Cartes Redessinées sans chevauchement) */}
          ...
          >
            ...
            <div className="mt-2 text-[10px] flex items-center gap-1 font-medium">
              {currentScoreData.isCompleteKingdom ? (
                ...
                <span className="text-slate-400">✗ {currentScoreData.emptyCells} case(s) vide(s)</span>
              )}
            </div>
          </button>
        </div>
      ...
      {/* 8. Liste Épurée des Domaines de Terrains */}
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className={`p-1 rounded-lg ${terrain.bgClass}`}>
                      <TerrainIcon type={dom.terrain} size={15} />
                    </div>
      ...
      {/* 9. Bouton Flottant / Fixe Podium Final */}
      <div className="pt-2">
        <button
          type="button"
          onClick={() => setIsPodiumOpen(true)}
          className="w-full py-3.5 px-4 rounded-2xl font-black text-sm flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-500 text-slate-950 shadow-xl shadow-amber-500/25 hover:scale-[1.01] active:scale-[0.99] transition-all"
        >
          <Trophy size={18} className="fill-slate-950" />
          <span>Voir le Podium & Vainqueur 🏆</span>
        </button>
      </div>
